...
const useFindRestaurant = ()=>{
    ...
    async function fetchRestaurantList(){
        const data = await fetch("https://corsproxy.org/?" + encodeURIComponent("https://www.swiggy.com/dapi/restaurants/list/v5?lat="+loc.coordinate.lat + "&lng="+loc.coordinate.lon + "&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING"));
        ...
        async function findRestaurantInJson(dataa) {
            if(dataa?.data?.communication?.swiggyNotPresent?.swiggyNotPresent === true){
                return "No_Res"
            }
